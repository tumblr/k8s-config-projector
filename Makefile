PACKAGE  = github.com/tumblr/k8s-config-projector
BINARY   = k8s-config-projector
...
DOCKER_IMAGE_BASE = tumblr/k8s-config-projector
DOCKER_TAG = $(VERSION)
...
check test tests: fmt lint vendor | ; $(info $(M) running $(NAME:%=% )tests…) @ ## Run tests
	$Q CGO_ENABLED=0 $(GO) test -timeout $(TIMEOUT)s $(ARGS) $(TESTPKGS)
